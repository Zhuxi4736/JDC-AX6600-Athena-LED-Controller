...
### v2.7.1（本次合并版 · 推荐一次性重装）
累积修复与新增功能，建议直接装此版，无需逐个试：

**新增**
- 🗑️ **删除已上传表情**：上传区每个 `.bin` 动画带「删」按钮，可移除不再需要的表情
- 🌤️ **天气显示**：状态机新增 `weather` 模块（自动 IP 定位 + 温度，默认状态已配置「天气 ↔ 时间(到秒)」轮换）
- 🕐 **时间到秒可自定义**：`time` 模块每行独立「显示到秒」勾选框（HH:MM 或 HH:MM:SS 自由切换）
- 🖤 **黑屏子状态**：子状态内容新增「黑屏（熄屏）」；**主屏熄灭但机身指示灯按该子状态独立亮灭**（可纯黑、可黑屏+时钟灯当夜灯等）
- 🔌 **HTTP 信号测试面板**：界面列出所有 `http` 规则的信号名，点「测试触发」即可模拟外部信号验证状态机

**修复**
- 📶 **上下行速率一直是 0**：改为实时采样 + 网卡自适应 fallback；默认网卡由 `br-lan` 改为 **`wan`**（真实出口），Luci 设置页已注明
- 👁️ **预览/试播无效**：状态机重写时屏蔽了预览消费，已修复（预览优先于状态机）
- 🖥️ **状态机 GUI 编辑器**：完整的图形化状态列表 + 弹窗（触发条件行式、子状态行式，全下拉/开关/数字框，少打字）

**状态机触发条件**：`http` / `cpu` / `mem` / `load` / `temp` / `time` / `wan` / `device` / `rate` / `idle` / `ipchange` / `script`，支持 `or` / `and` 组合。
...
#### 保持态（信号存在期间一直显示，撤销才结束）

普通 `http` 信号是**瞬态**：推一次，状态亮 60 秒就自动回默认（适合"门铃响一下"这种一次性事件）。

如果你的状态想**在信号存在的整段时间都显示、信号消失自动结束**（例如「Hermes 思考中」动画——思考开始亮、思考结束灭），需要在状态机 GUI 的触发规则里把该 `http` 规则的**「保持态」勾选框**打开。此时：
- `GET /signal/<信号名>` → 信号**激活**，对应状态一直显示（抢占默认）
- `GET /signal/<信号名>/off` → 信号**撤销**，对应状态立刻让出，回默认或下一优先级

```bash
# Hermes 开始思考：点亮「思考中」动画
curl "http://192.168.1.1:8377/signal/thinking"

# Hermes 思考结束：熄灭「思考中」动画
curl "http://192.168.1.1:8377/signal/thinking/off"
```

> 任何程序都能用：AI Agent、脚本、HA、手机快捷指令——只要能在开始/结束两个时刻各发一个 HTTP 请求即可。无需改代码，纯配置驱动。
